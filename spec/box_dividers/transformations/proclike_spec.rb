...
module BoxDividers::Transformations
  RSpec.describe Proclike do
    ...
    let(:point_return_transformer) { ->(p) { BoxDividers::Point.new(p.x + 10, p.y + 10) } }
    let(:input_point) { BoxDividers::Point.new(1,2) }
    let(:expected_point) { BoxDividers::Point.new(11,12) }
    ...
    describe "running the transformation with #call() and passing in a Point" do
      it "returns the result as a Point if the block returns an [x,y] tuple" do
        expect(Proclike.new(array_return_transformer).call(input_point)).
          to eq(BoxDividers::Point.new(2,4))
      end
    end
    ...
    context "composition" do
      ...
      let(:expected_point) { BoxDividers::Point.new(12,14) }
