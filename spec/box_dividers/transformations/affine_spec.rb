...
module BoxDividers::Transformations
  RSpec.describe Affine do
    ...
    let(:result_point) { BoxDividers::Point.new(-1,-2) }
    ...
    specify "two Affine transforms can be composed into a single one" do
      ...
      expect(t2.compose(t1).call(input_point)).to eq(BoxDividers::Point.new(-1,-2))
    end
    ...
    specify "an Affine transform can be composed with a Proclike" do
      ...
      expect(t2.compose(t1).call(input_point)).to eq(BoxDividers::Point.new(3,-4))
    end
  end
end
